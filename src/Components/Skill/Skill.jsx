...
import { SiJavascript, SiReact, SiCss3, SiHtml5,SiCplusplus } from "react-icons/si";
...
function Skill() {
  ...
  return (
    <div className="w-full h-screen  flex flex-col items-center pt-13 md:pt-7 bg-[#080809]">
      <h2 className="text-2xl text-[#f2f3fc] font-Merriweather font-bold mb-6 pt-2">Skills</h2>
      <div className=" w-4/5 h-2/5 md:w-2/3 md:h-fit flex flex-wrap justify-center  overflow-auto md:flex-row md:justify-around  py-4 px-8">
        {BigSkills.map((skill) => {
          ...
          return (
            <div
              className="group shadow-md bg-white flex m-1 flex-col justify-around w-fit py-1.5 px-1 shadow-gray-400  rounded-2xl"
              key={skill.name}
            >
              <p className="bg-gray-800 text-white text-center opacity-0 group-hover:opacity-100 transition-all ease-in duration-500 delay-300  rounded-full  ">
                {skill.name}
              </p>
              <Icon
